...
if [[ $_arg_video =~ (.*\/)([0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2})-.*.mp4$ ]]; then
  ...
  _front=${_path}${_prefix}-front.mp4
  _right=${_path}${_prefix}-left_repeater.mp4
  _left=${_path}${_prefix}-right_repeater.mp4
  _files="$_front $_right $_left"
  _target_length=0
  for v in $_files; do
    if [[ -f $v ]]; then
      _length=$(printf "%.0f" $(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 $v))
      if [ $_target_length -eq 0 ]; then
        _target_length=$_length
      fi
      if [ $_length -ge $_target_length ] && [ $_length -le $_target_length ]; then
        echo "  $v valid"
      else
  ...
  _lengthc=$(printf "%.0f" $(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 $_crunch))
    ...
    _lengthh=$(printf "%.0f" $(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 $_highlight))
    if [ $_lengthh -eq 0 ]; then
      echo "  Highlight reel == $_lengthh seconds, clearing"
      rm $_highlight
    fi
    echo "  Crunched $_crunch!  Removing originals..."
    rm $_front; rm $_right; rm $_left
  else
    echo "  Crunch failed :("
  fi
else
