...
# ARG_POSITIONAL_INF([files],[one or more (with wildcard) files to highlight])
# ARGBASH_GO()
...
print_help()
{
	...
	printf 'Usage: %s [-h|--help] [-t|--threshold <arg>] [-f|--framerate <arg>] [<files-1>] ... [<files-n>] ...\n' "$0"
...
for f in "$_arg_files"; do
  if [[ $f =~ (.*)\.mp4$ ]]; then
    ...
    ffmpeg -y -i $f -nostdin -loglevel panic -filter \
      ...
      -pix_fmt yuv420p ${_output}-highlight.mp4
  fi
done
